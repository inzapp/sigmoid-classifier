...
class CosineLRDecay(tf.keras.callbacks.Callback):
    ...
    def update(self, model):
        self.model = model
        lr = self.min_lr + 0.5 * (self.max_lr - self.min_lr) * (1.0 + np.cos(((1.0 / (0.5 * self.cycle_length)) * np.pi * self.batch_cnt) + np.pi))
        tf.keras.backend.set_value(self.model.optimizer.lr, lr)
        self.batch_cnt += 1
        if self.batch_cnt % self.cycle_length == 0:
            self.save_model()

    def save_model(self):
        if self.train_data_generator_flow is None or self.validation_data_generator_flow is None:
            self.model.save(f'checkpoints/model_{self.batch_cnt}_batch.h5', include_optimizer=False)
        else:
            ...
            if val_acc > self.max_val_acc:
                ...
                print(f'{self.batch_cnt} batch => acc: {acc:.4f}, val_acc: {val_acc:.4f}\n')
                self.model.save(f'checkpoints/model_{self.batch_cnt}_batch_acc_{acc:.4f}_val_acc_{val_acc:.4f}.h5', include_optimizer=False)
